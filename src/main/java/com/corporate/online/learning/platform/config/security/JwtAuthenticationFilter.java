...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        final String accountEmail = jwtService.extractUsername(jwtToken);
        if (!ObjectUtils.isEmpty(accountEmail)
            ...
            var isTokenValidDb = tokenRepository.findByToken(jwtToken)
                    .map(token -> !token.getExpired())
                    .orElse(false);

            if (jwtService.isTokenValid(jwtToken, userDetails) && isTokenValidDb) {
